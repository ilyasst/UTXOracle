...
# if date mode search for all the blocks on this day
    ...
    # Fetch block headers in parallel (same approach as -rb mode) to avoid
    # hanging over high-latency RPC connections. Cap at block_count so running
    # with today's date via -d does not try to fetch non-existent future blocks.
    from concurrent.futures import ThreadPoolExecutor

    _day_workers = 8 if _use_public_api else 4
    _search_end = min(price_day_block + 300, block_count + 1)
    _day_heights = list(range(price_day_block, _search_end))

    _day_results = {}
    with ThreadPoolExecutor(max_workers=_day_workers) as _ex:
        for _h, _res in zip(_day_heights, _ex.map(get_block_time, _day_heights)):
            _day_results[_h] = _res

    print_next = 0
    price_day_block_end = price_day_block
    for _i, _h in enumerate(sorted(_day_results.keys())):
        _t, _bhash = _day_results[_h]
        if (
            len(_day_heights) > 0
            and _i * 100 // len(_day_heights) >= print_next
            and print_next < 100
        ):
            print(str(print_next) + "%..", end="", flush=True)
            print_next += 20
        if get_day_of_month(_t) == day1:
            block_nums_needed.append(_h)
            block_hashes_needed.append(_bhash)
            block_times_needed.append(_t)
            price_day_block_end = _h + 1
        else:
            break
...
_block_cache_dir = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), ".block_cache"
)
os.makedirs(_block_cache_dir, exist_ok=True)


def _cache_path(bh):
    return os.path.join(_block_cache_dir, bh)
...
_prefetch_workers = (
    4 if _use_public_api else 1
)  # parallel only for public APIs (node RPC is single-threaded)
...
print(
    f"\nPrefetching {len(block_hashes_needed)} blocks ({_cache_hits} cached, {_to_fetch} to fetch, {_prefetch_workers} workers)...",
    flush=True,
)
...
try:
    _cached_files = sorted(
        os.listdir(_block_cache_dir),
        key=lambda f: os.path.getmtime(os.path.join(_block_cache_dir, f)),
    )
...
# Use last known price from file to seed the slide and avoid harmonic basins
try:
    import os, math

    last_price_file = "/home/openkaz/bitcoin/.last_btc_price"
    if os.path.exists(last_price_file):
        with open(last_price_file, "r") as f:
            last_p = int(f.read().strip())
        expected_slide = int((math.log10(100 / last_p) - (-3)) * 200)
        # restrict slide to +/- 40 bins (which is +58% / -37% price range)
        min_slide = max(-141, expected_slide - 40)
        max_slide = min(201, expected_slide + 40)
except Exception:
    pass
...
if block_mode:
    print("\n\n\t\tBlock window price: $" + f"{int(central_price):,}\n\n", flush=True)
